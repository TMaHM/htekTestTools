...
class Phone(TestUrl):
    ...
    def acd(self, method: str = None, solution: str = None, aid: int = 0):
        if method.lower() in ('in', 'out',):
            method = method.upper()
            url_acd_method = '{drd_pre}ACD:LOG{method}:aid=0'.format(drd_pre=self.drd_prefix, method=method)
        elif method.lower() in ('available', 'wrap up'):
            method = method.upper()
            url_acd_method = '{drd_pre}ACD:{method}:aid=0'.format(drd_pre=self.drd_prefix, method=method)
