        ...
        if self.check_status('ringing') is True:
            ...
            if r_answer[0] == 200:
                ...
                if self.check_status('talking') is True:
                    log.info('%s(%s) now executing answered by %s.' % (self.ext, self.ip, cmd.upper()))
                    self.keep_call(1)
                    log.info('%s(%s) execute function [Answer] done.' % (self.ext, self.ip))
                    return 200
                else:
                    log.error('Check status failed...But the scripts will continue.')
                    return 400
            else:
                log.error('%s(%s) answer failed.' % (self.ext, self.ip))
                return 500
