...
class Phone(TestUrl):
    ...
    def requests_get(self, url: str, func_name):
        ...
        self.log.error('Auth Failed on %s, Return code is %s, please verify it ...' % (self.ip, r.status_code))
    ...
    def set_idle_status(self):
        ...
        if self.check_status('idle'):
            self.log.info('%s is already on [idle] status.' % self.ext)
            return 200
        else:
            if r_return_idle[0] == 200:
                self.log.info('%s set [idle] status success.' % self.ip)
            elif r_return_idle[0] == 404:
                self.log.error('%s has not merged, try press [Speaker] to return to idle.' % url_return_idle)
                ...
                self.log.info('%s return 401, please check.' % self.ip)
            else:
                self.log.error('%s return %s, set [idle] status failed.' % (url_return_idle, r_return_idle[0]))
                return 400
