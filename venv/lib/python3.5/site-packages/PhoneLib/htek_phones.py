...
class Phone(TestUrl):
    ...
    def keep_call(self, seconds: int):
        ...
        :return: 返回keep的时间
        """
        log.info('%s keep the call on for %s seconds.' % (self.ext, seconds))
        time.sleep(seconds)
        return seconds
    ...
    def flexible_seating(self, aid: int = 0, number: str = Null, pwd: str = Null, method: str = Null, ):
        """
        ...
        if method.upper() is 'In':
            url_flx_s = self.drd_prefix + 'GUESTIN:aid=%s&number=%s&password=%s' % (aid, number, pwd)
        elif method.upper() is 'OUT':
            url_flx_s = self.drd_prefix + 'GUESTOUT'
        else:
        ...
        if r_flex_s[0] == 200:
            ...
            # 临时图片路径
            temp_img = '%s/temp/flx_img_temp.jpg' % log.screen_dir
            # 正确图片路径
            standard_img = '{path}/standard/drd_flx_{method}_img.jpg'.format(path=log.screen_dir, method=method.lower())
            # 如果截屏成功，打开后进行裁剪，存为临时文件并和正确的图片进行对比
            if captured_img.isdigit() is not True:
                img = Image.open(captured_img)
                # crop的坐标是待确定的值,(top, left, bottom, right)
                cropped = img.crop((0, 0, 128, 128))
                cropped.save(temp_img)
                result = compare_img(temp_img, standard_img)
                if result == 0:
    ...
    def screen_shot(self, screen_name):
        ...
        :return: 成功返回包含完整路径的文件名 or 401 Auth Failed or 404 Not Found or 500 Connection Error
        """
        retry = 0
    ...
    def reboot(self):
        ...
        """
        url_restart = self.keyboard + 'Reboot'
...
def compare_img(img_file, img_original):
    """
    对比原理：
    ...
    h_new = img1.histogram()
    h_ori = img2.histogram()
