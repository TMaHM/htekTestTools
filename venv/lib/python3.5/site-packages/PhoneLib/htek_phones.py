...
class Phone(TestUrl):
    ...
    def flexible_seating(self, method: str = None, solution: str = None, aid: int = 0, number: str = None,
        ...
        if r_flex_s[0] == 200:
            ...
            # 正确图片路径
            standard_img = '{path}/{solution}_{model}_flx_{method}_img.jpg'.format(path=IMG_STANDARD_PATH,
                                                                                   solution=self.solution,
                                                                                   model=self.model.lower(),
                                                                                   method=method.lower())
            # 截屏裁剪部分相对于截屏的像素点元组
            crop_pixel = (0, 0, 129, 129)
            # 如果截屏成功，打开后进行裁剪，存为临时文件temp_img并和正确的图片进行对比
            if captured_img.isdigit() is not True:
                temp_img = cropping_img(captured_img, crop_pixel)
    ...
    def acd(self, method: str = None, solution: str = None, aid: int = 0):
        if method.lower() in ('in', 'out', 'available', 'wrap up'):
            method = method.upper()
            url_acd_method = '{drd_pre}ACD:{method}:aid=0'
        elif method.lower() in ('unavailable', 'dispcode'):
            method = method.upper()
...
def cropping_img(crop_img, pixel_tuple):
    """
    打开截屏文件，裁剪后返回存放路径
    ...
    tmp_file_path = cropped.save(IMG_TEMP_PATH)
