...
IMG_TEMP_PATH = '{cur_dir}/{img_dir}'.format(cur_dir=PROJECT_ROOT_PATH, img_dir='img/temp')
if not os.path.exists(IMG_TEMP_PATH):
    os.makedirs(IMG_TEMP_PATH)

IMG_STANDARD_PATH = '{cur_dir}/{img_dir}'.format(cur_dir=PROJECT_ROOT_PATH, img_dir='img/standard')
...
acd_dir = {
    'uc926':{'drd':(386, 4, 414, 34)},
    'uc924e':{'drd':(379, 10, 402, 38)}
}
...
class TestUrl:
    ...
    def __init__(self, ip, usr='admin', pwd='admin'):
        ...
        self.prefix = 'http://{usr}:{pwd}@{ip}'.format(usr=self.usr, pwd=self.pwd, ip=self.ip)
        ...
        self.drd_prefix = self.prefix + '/AutoTest&drd='
